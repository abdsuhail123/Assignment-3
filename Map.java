...
public class Map{
	...
	//   private Encounter[][] encounters; // may have null entries. 
  	...
  	public Map(int rows, int cols){
  		...
  		// Add this
  		StdDraw.setCanvasSize(rows * tileSize, cols * tileSize);
  	}

  	// Generate the map based on the template
  	public Map(char[][] mapTemplate){
		currentMap = mapTemplate;
		if( mapTemplate != null && mapTemplate.length > 0){
			StdDraw.setCanvasSize(mapTemplate.length * tileSize, mapTemplate[0].length * tileSize);
		}else{
			System.out.println("Error detected when initializing a Map from a template.");
		}
	...
	public static void main(String[] args){
		...
		// Depreciated: This has been added to the Map Constructor. 
		// StdDraw.setCanvasSize(5 * m.tileSize, 6 * tileSize);
